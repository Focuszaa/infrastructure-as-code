...
class EventTrigger(Stack):
    def __init__(self, app: App, id: str, props, **kwargs) -> None:
        ...
        cw_event_service_role = iam.Role(
            self, "Role",
        ...
        CfnOutput(
            self, "Output-Event-service-role",
            description="Event-service-role",
            value=cw_event_service_role.role_arn
        )
        CfnOutput(
            self, "Output-cc_change_rule",
            description="codecommit change event rule",
            value=cc_change_rule.rule_arn
        )
        CfnOutput(
            self, "Output-cw_schedule_weekly_rule",
            description="schedule weekly event rule",
            value=cw_schedule_weekly_rule.rule_arn
        )
